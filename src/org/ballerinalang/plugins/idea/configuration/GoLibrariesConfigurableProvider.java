...
import org.ballerinalang.plugins.idea.project.GoProjectLibrariesService;
    ...
    private Configurable createConfigurable(boolean dialogMode) {
        return new CompositeConfigurable<UnnamedConfigurable>() {
            ...
            protected List<UnnamedConfigurable> createConfigurables() {
                ...
                String[] urlsFromEnv = ContainerUtil.map2Array(BallerinaSdkUtil.getGoPathsRootsFromEnvironment(), String.class,
                        VirtualFile::getUrl);
                result.add(new BallerinaLibrariesConfigurable("Global libraries", GoApplicationLibrariesService.getInstance(), urlsFromEnv));
                if (!myProject.isDefault()) {
                    result.add(new BallerinaLibrariesConfigurable("Project libraries", GoProjectLibrariesService.getInstance(myProject)));
...
//                            return new BallerinaLibrariesConfigurable("Module libraries", GoModuleLibrariesService.getInstance(module));
//                        }
//                    });
                }
                return result;
