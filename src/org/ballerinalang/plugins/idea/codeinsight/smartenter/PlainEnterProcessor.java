...
//        getEnterHandler(IdeActions.ACTION_EDITOR_START_NEW_LINE).execute(editor, ((EditorEx) editor).getDataContext());
    ...
    static boolean expandCodeBlock(Editor editor, PsiElement psiElement) {
        ...
        if (firstElement == null) {
            ...
            // Plain enter processor inserts enter after the end of line, hence, we don't want to use it here because
            // the line ends with
...
//        enterHandler.execute(editor, ((EditorEx) editor).getDataContext());
    ...
    private static boolean processExistingBlankLine(@NotNull Editor editor, @Nullable PsiCodeBlock codeBlock,
        ...
        if (codeBlock == null) {
            ...
            if (node == null || node.getElementType() != BallerinaTypes.LBRACE) {
                return false;
            }
        }
