...
public class BallerinaUtil {
    ...
    public static List<BallerinaFunctionName> findFunctionName(Project project, String functionName) {
        List<BallerinaFunctionName> result = new ArrayList<BallerinaFunctionName>();
        Collection<VirtualFile> virtualFiles =
                FileBasedIndex.getInstance().getContainingFiles(FileTypeIndex.NAME, BallerinaFileType.INSTANCE,
                        GlobalSearchScope.allScope(project));
        for (VirtualFile virtualFile : virtualFiles) {
            BallerinaFile ballerinaFile = (BallerinaFile) PsiManager.getInstance(project).findFile(virtualFile);
            if (ballerinaFile != null) {
                BallerinaFunctionDefinition[] functionDefinitions = PsiTreeUtil.getChildrenOfType(ballerinaFile,
                        BallerinaFunctionDefinition.class);
                if (functionDefinitions != null) {
                    for (BallerinaFunctionDefinition functionDefinition : functionDefinitions) {
                        ASTNode childByType = functionDefinition.getNode().findChildByType(
                                BallerinaTypes.FUNCTION_NAME);
                        if (childByType == null) {
                            continue;
                        }
                        if (functionName.equals(childByType.getText())) {
                            result.add(functionDefinition.getFunctionName());
                        }
                    }
                }
            }
