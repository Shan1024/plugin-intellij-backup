...
import org.ballerinalang.plugins.idea.configuration.GoLibrariesConfigurableProvider;
...
public class GoModuleLibrariesInitializer implements ModuleComponent {
    ...
    private static final String GO_LIB_NAME = "BALLERINA_REPOSITORY";
...
//    };
    ...
    public void moduleAdded() {
        if (!myModuleInitialized) {
            ...
            myConnection.subscribe(BallerinaLibraryService.LIBRARIES_TOPIC, newRootUrls -> scheduleUpdate());
...
//            VirtualFileManager.getInstance().addVirtualFileListener(myFilesListener);
    ...
    private static void showNotification(@NotNull Project project) {
        ...
        if (!shownAlready) {
            ...
            Notification notification = BallerinaConstants.BALLERINA_NOTIFICATION_GROUP.createNotification("GOPATH " +
                            "was detected",
...
//            if (GoSdkService.getInstance(project).isGoModule(myModule)) {
                synchronized (myLastHandledGoPathSourcesRoots) {
                    Collection<VirtualFile> goPathSourcesRoots = BallerinaSdkUtil.getGoPathRoots(project, myModule);
...
//                    if (!myLastHandledGoPathSourcesRoots.equals(goPathSourcesRoots) || !myLastHandledExclusions.equals(excludeRoots)) {
                        ...
                        ApplicationManager.getApplication().invokeLater(() -> {
                            if (!myModule.isDisposed() && true/*GoSdkService.getInstance(project).isGoModule(myModule)
                            */) {
